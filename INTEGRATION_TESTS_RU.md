...
8.5 autoPathHeader.customTemplatesByExtension:
- Пользователь может задать кастомный шаблон комментария для конкретного **расширения файла** (включая составные, например `.env.local`).
- Формат настройки:
  ```json
  ...
  ```
Приоритет:
- customTemplatesByExtension[расширение]
- formatTemplate (если задан)
- Стандартный шаблон языка ({comment})
- Расширение определяется через path.extname(filePath) (включая точку), регистронезависимо.

8.5.1 Приоритет настроек:

- Заданы одновременно:
```json
"autoPathHeader.formatTemplate": "// GLOBAL: {path}",
"autoPathHeader.customTemplatesByExtension": { ".test.ts": "// SPECIFIC: {path}" }
```
- Для .test.ts → используется `// SPECIFIC: ...`
- Для .regular.ts → используется `// GLOBAL: ...`

8.5.2 Отсутствие совпадения по расширению:

- Файл script.py — расширение .py не указано в customTemplatesByExtension
- Используется formatTemplate или стандартный шаблон
...
- Кастомный шаблон для нестандартного расширения (например, Makefile.local):
- Убедиться, что расширение определяется как .local (а не Makefile.local целиком)
- Если нужно — уточнить логику определения расширения (но path.extname вернёт .local для Makefile.local, что может быть неожиданно; при необходимости — документировать или использовать альтернативную стратегию)
