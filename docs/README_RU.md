...
Расширение активируется сразу после установки для обработки файлов с любым расширением. Оно отслеживает события открытия файлов и автоматически вставляет комментарии с путями в соответствии с вашими настройками.
...
Чтобы не вставлять комментарии в файлах с определёнными расширениями (например, лог-файлы или временные файлы), добавьте их в настройку `disabledExtensions`:
...
### Кастомные шаблоны для расширений файлов

Вы можете определить кастомные шаблоны для конкретных расширений файлов. Это позволяет использовать разный формат для разных типов файлов. Расширение определяется через path.extname(filePath) (включая точку), регистронезависимо. Поддерживаются составные расширения, такие как `.env.local`, а также конкретные имена файлов, такие как `Dockerfile.dev`.

Приоритет выбора шаблона:
1. `customTemplatesByExtension[конкретное имя файла]` (например, "Dockerfile.dev")
2. `customTemplatesByExtension[составное расширение]` (например, ".env.local")
3. `customTemplatesByExtension[обычное расширение]` (например, ".ts")
4. `formatTemplate` (если задан)
5. Стандартный формат комментария языка

```jsonc
{
  "autoPathHeader.customTemplatesByExtension": {
    ".env.local": "# ЛОКАЛЬНАЯ НАСТРОЙКА — {path}",
    ".test.ts": "// 🧪 ТЕСТ: {path}",
    "Dockerfile.dev": "# DEV СБОРКА: {path}"
  }
}
```

Поддерживаемые плейсхолдеры для кастомных шаблонов включают `{path}`, `{filename}` и `{dirname}`, которые будут заменены на актуальные значения при вставке комментария.
