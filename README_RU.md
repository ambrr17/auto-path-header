...
- ✅ Автоматическое обновление пути при переименовании/перемещении файлов
- ✅ Ручная вставка комментария через Command Palette
...
- `autoPathHeader.formatTemplate` - шаблон комментария. Поддерживает плейсхолдеры `{comment}`, `{path}`, `{prefix}`, `{suffix}`.
- `autoPathHeader.disabledLanguages` - список languageId VS Code, для которых автодействия отключены.
