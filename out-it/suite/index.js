...
function run() {
    const mocha = new mocha_1.default({ ui: 'tdd', color: true, timeout: 10000, reporter: CustomReporter });
    const testsRoot = path.resolve(__dirname, '..'); // Use the parent directory (out-it) to find the individual tests
    return new Promise(async (resolve, reject) => {
        try {
            const files = await (0, glob_1.glob)('individual-tests/**/*.it.test.js', { cwd: testsRoot }); // Look for tests in the parent directory, in the individual-tests subdirectory
